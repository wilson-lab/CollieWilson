...
%   angular      - Output variable to compare against (e.g., angular velocity, spike rate)
...
%
function [pulseTrials, pulseMeans] = pulse_v_onlyturns(panelps,forward,angular,ttime,pulseSelect,pulseOptions,nSweep,runSelect)
%% initialize
    ...
    try
        ...
        if size(thisOrderR,2)==nSweep
            ...
            for p = 1:nSweep
                ...
                % conditional storage for pulseVarR
                if p > (nSweep / 2)
                    if mean(thisAngular(thisOrderR(:,p))) > 0
                        pulseVarR(:,c,p) = thisAngular(thisOrderR(:,p));
                    else
                        pulseVarR(:,c,p) = NaN(size(thisOrderR(:,p)));
                    end
                else
                    if mean(thisAngular(thisOrderR(:,p))) < 0
                        pulseVarR(:,c,p) = thisAngular(thisOrderR(:,p));
                ...
                % conditional storage for pulseVarL (opposite logic of pulseVarR)
                if p < (nSweep / 2)
                    if mean(thisAngular(thisOrderL(:,p))) < 0
                        pulseVarL(:,c,p) = thisAngular(thisOrderL(:,p));
                    else
                        pulseVarL(:,c,p) = NaN(size(thisOrderL(:,p)));
                    end
                else
                    if mean(thisAngular(thisOrderL(:,p))) > 0
                        pulseVarL(:,c,p) = thisAngular(thisOrderL(:,p));
...
for p = 1:nSweep
    ...
    if sum(idxR)>=minTrials
        meanVarOutR(:,1,p) = mean(pulseVarR(:,idxR,p),2,"omitnan");
    else
    ...
    if sum(idxL)>=minTrials
        meanVarOutL(:,1,p) = mean(pulseVarL(:,idxL,p),2,"omitnan");
    else
