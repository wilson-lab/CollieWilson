% FUNCTION: spikert_nonlinear
%
% DESCRIPTION:
%   This function fits two models to predict the firing rate of a neuron based on visual and behavioral inputs. 
%   The linear model uses forward and angular velocities as predictors, while the truncated Gaussian model 
%   uses visual object position. Cross-validation is applied for each model, and the mean R-squared values 
%   across folds are calculated.
%
% INPUTS:
%   spikert             - Firing rate data (array) with NaNs for missing values.
%   panelps             - Visual object position (array) with NaNs for hidden times.
%   forward             - Forward velocity data (array).
%   angular             - Angular velocity data (array).
%   ttime               - Time vector (array).
%   peak_position       - Peak position for the truncated Gaussian model.
%   spread              - Spread parameter for the truncated Gaussian model.
%
% OUTPUTS:
%   R2_linear           - Mean R-squared value across folds for the linear model.
%   R2_truncatedGaussian - Mean R-squared value across folds for the truncated Gaussian model.
%   mdl_linear          - Final linear model fit using all data.
%   mdl_truncatedGaussian - Final truncated Gaussian model fit using all data.
%
