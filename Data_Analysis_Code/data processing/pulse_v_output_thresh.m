% pulse_v_output_thresh
% This data processing function analyzes the relationship between each motion pulse and the corresponding 
% output (e.g., angular velocity or spike rate). The function includes a pre-trial threshold requirement 
% (e.g., turning or firing rate > 0) to ensure only valid trials are analyzed. This function builds upon 
% pulse_v_output but adds the behavioral threshold to refine the analysis.
%
% INPUTS:
%   panelps      - Panel positions (degrees)
%   forward      - Forward velocity (mm/s)
%   varOut       - Output variable to compare against (e.g., angular velocity or spike rate)
%   varThresh    - Variable to threshold against (e.g., firing rate threshold)
...
%
function [pulseTrials, pulseMeans] = pulse_v_output_thresh(panelps,forward,varOut,varThresh,ttime,pulseSelect,pulseOptions,nSweep,runSelect)
%% initialize
...
%% fetch behavioral index
% optional threshold for running behavior
...
% threshold for variable index
threshIdx = varThresh>5;
    ...
    try
        ...
        if size(thisOrderR,2)==nSweep
            ...
            for p = 1:nSweep
                ...
                threshIdxR(:,c,p) = threshIdx(thisOrderR(:,p),t);
                threshIdxL(:,c,p) = threshIdx(thisOrderL(:,p),t);
            end
        ...
        for p = 1:nSweep
            ...
            threshIdxR(1:winIdx,c,p) = zeros(winIdx,1);
            threshIdxL(1:winIdx,c,p) = zeros(winIdx,1);
        end
        c = c+1; %update counter
    end
end
...
prePulseDur = round(pulseDur/4);

%% determine which trials met behavioral requirements (if any) for this run
% for behavior threshold, assess behavior during full sweep duration
minIdx = round(pulseDur*minInclusion1);
goodBehaveR = sum(runIdxR,1)>minIdx;
goodBehaveL = sum(runIdxL,1)>minIdx;

% for variable threshold, assess during pre-sweep interval only
minIdx = round(prePulseDur*minInclusion2);
goodPreR = sum(threshIdxR(1:prePulseDur,:,:),1)>minIdx;
goodPreL = sum(threshIdxL(1:prePulseDur,:,:),1)>minIdx;

% fetch trials that met both conditions
goodTrialsR = goodBehaveR & goodPreR;
goodTrialsL = goodBehaveL & goodPreL;
