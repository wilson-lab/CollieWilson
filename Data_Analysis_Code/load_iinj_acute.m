...
% the fly was presented with a blank environment, and acute current injections 
% were delivered to the cell, alternating between positive and negative currents.
% This script manages the initialization, data loading, and preprocessing 
% for individual experiments, as well as the pooled analysis pipeline.
%
% Inputs:
...
%   3. Executes preprocessing for individual experiments (via preprocess_iinj_acute).
%   4. Runs the pooled analysis pipeline for all experiments (via pipeline_iinj_acute).
%
% Outputs:
%   - Processed data is saved into the specified experiment folder.
%
function load_iinj_acute(thisRun,userClear)
%% load in all experiments of interest

% pick which battery analysis to run through pipeline
switch thisRun
    case '19'
    exptFolder = 'AOTU019 IInj Acute';
    exc_name = 'AOTU019_Tracker.xlsx'; %excel file containing meta data
    exc_sheet = 6; %sheet for this experiment
    case '25'
    exptFolder = 'AOTU025 IInj Acute';
...
for nf = 1:nExpt
    ...
    preprocess_iinj_acute(exptFolder,thisFly,trackLR(nf))
end
...
pipeline_iinj_acute(exptFolder)
